...
cb.PARSEPREFIX = " \
    const Mandelfield = (x, y) => { \
        let a = x, b = y; \
        for (let i = 0; i < 16; i++) { \
            let aa = a*a - b*b, bb = 2*a*b; \
            a = aa + x, b = bb + y; \
        } \
        return a*a + b*b < 4 ? 1 : -1; \
    }; \
";
const F = cb.vect.field(
    "-y / Math.sqrt(x*x + y*y) * Mandelfield(x, y)",
    " x / Math.sqrt(x*x + y*y) * Mandelfield(x, y)"
);
class Particle {
    constructor(p) {
        this.pos = p;
